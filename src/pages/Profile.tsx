  ...
  return (
    ...
    >
          ...
          {isEditingName ? (
            ...
            <div className="flex flex-col items-center justify-center mb-1 group cursor-pointer" onClick={handleEditName}>
              ...
              <div className="inline-flex items-center gap-1.5 px-3 py-1.5 mt-2 rounded-full bg-slate-100/80 dark:bg-slate-800/80 backdrop-blur-sm border border-slate-200/50 dark:border-slate-700/50">
                <ShieldHalf className="w-3.5 h-3.5 text-emerald-500" />
                <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">{profile?.email || user?.email || 'Unavailable'}</p>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-2 gap-3 mb-6">
        <div className="bg-white/70 dark:bg-slate-800/40 backdrop-blur-xl rounded-[24px] p-4 shadow-sm border border-white dark:border-slate-700/50 flex flex-col items-center justify-center text-center group cursor-default transition-all hover:scale-[1.02]">
          <div className="w-10 h-10 rounded-full bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-blue-500 mb-2">
            <Check className="w-5 h-5" />
          </div>
          <span className="text-[20px] font-black text-slate-800 dark:text-white leading-none mb-1">100%</span>
             ...
             <div className="flex justify-between items-center bg-white dark:bg-slate-900/80 p-4 rounded-2xl shadow-[0_2px_10px_rgb(0,0,0,0.02)] border border-slate-100 dark:border-slate-800 group/box hover:border-indigo-100 dark:hover:border-indigo-500/30 transition-colors">
               <div className="flex flex-col text-left">
                 ...
                 <div className="flex items-center gap-2 flex-wrap">
                   <span className="font-display font-semibold text-indigo-600 dark:text-indigo-400 text-lg tracking-tight select-all">{profile?.myReferCode || 'Unavailable'}</span>
                   {profile?.myReferCode && !/^[A-Z]{2}[0-9]{6}$/.test(profile.myReferCode) && (
                     <button 
                       onClick={upgradeReferralCode}
                       disabled={upgradingCode}
                       className="p-1 px-2 rounded-lg bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 active:scale-95 transition-all text-[11px] font-bold flex items-center gap-1 cursor-pointer"
          ...
          <div className="flex items-center justify-between p-4 rounded-[24px]">
            ...
            <div className="bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 px-4 py-2 rounded-2xl shadow-inner">
              <span className="font-black tracking-tighter text-slate-800 dark:text-white text-lg">
                ৳ {((profile?.balances?.main || 0) + (profile?.balances?.bonus || 0) + (profile?.balances?.referral || 0) + (profile?.balances?.gift || 0) + Object.values(profile?.balances?.tasks || {}).reduce((a, b) => (a as number) + (b as number), 0)).toFixed(2)}
              </span>
            </div>
          </div>
