...
code = code.replace(
  /<div>\s*<label className="block text-\[11px\] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest pl-1 mb-1.5">\{t\('select_method'\) \|\| 'Method'\}<\/label>\s*<select[\s\S]*?<\/select>\s*<\/div>/,
  formReplacement
);
